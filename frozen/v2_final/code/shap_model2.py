...
m = LGBMClassifier(n_estimators=800, learning_rate=0.03, num_leaves=127,
                   min_child_samples=40, subsample=0.85, subsample_freq=1,
                   colsample_bytree=0.8, reg_lambda=5.0, scale_pos_weight=spw,
                   random_state=42, n_jobs=-1, verbose=-1).fit(tr[feats], tr.y.values)
...
GROUPS = {
 "Recent dengue incidence": ["casos","p_inc100k","casos_lag_1","casos_lag_2","casos_lag_4","casos_lag_8",
                             "casos_roll4_mean","casos_roll8_mean","casos_growth_1","casos_growth_4",
                             "inc_roll4","Rt","Rt_lag_1","Rt_lag_2","Rt_lag_4"],
 "Rainfall":     ["precip_total_semana","precip_media_semana","precip_max_semana","dias_lluvia_semana",
                  "precip_total_semana_lag_1","precip_total_semana_lag_4","precip_roll4_sum","precip_roll8_sum"],
 "Temperature":  ["tempmin","tempmed","tempmax","tempmed_lag_1","tempmed_lag_4",
                  "tempmed_roll4_mean","tempmed_roll8_mean","LST_Day_mean","LST_Night_mean"],
 "Humidity":     ["umidmin","umidmed","umidmax","umidmed_lag_1","umidmed_lag_4"],
 "Vegetation (NDVI/EVI)": ["NDVI_mean","NDVI_std","EVI_mean","EVI_std"],
 "Land cover":   ["pct_tree_cover","pct_builtup","pct_cropland","pct_water","pct_grassland","elev_mean"],
...
for _, r in gd.iterrows():
    bar = "#" * int(round(28 * r.mean_abs_shap / mx))
...
plt.tight_layout(); plt.savefig("reports/model2_shap_summary.png", dpi=140); plt.close()
gd.set_index("driver")["pct_of_total"].sort_values().plot(kind="barh", figsize=(8,5))
plt.xlabel("% of total |SHAP|"); plt.title(f"Dengue outbreak drivers ({HORIZON*7}-day forecast)")
plt.tight_layout(); plt.savefig("reports/model2_shap_drivers.png", dpi=140); plt.close()
json.dump({"horizon_weeks": HORIZON, "drivers": gd.to_dict("records"),
           "top_features": mean_abs.head(30).to_dict()},
          open("reports/model2_shap.json","w"), indent=2, default=float)
