...
st.set_page_config(page_title="DengueShield", page_icon=":material/coronavirus:", layout="wide")
...
def probability_bar(p: float, thr: float, label: str):
    ...
    fig.add_shape(type="line", x0=thr, x1=thr, y0=-0.25, y1=1.25, line={"color": INK, "width": 2})
    fig.add_annotation(
        x=thr,
        y=1.75,
        text=f"threshold {thr:.0%}",
        showarrow=False,
        font={"size": 11, "color": MUTED, "family": "IBM Plex Mono"},
    )
    fig.add_annotation(
        x=0, y=-1.1, text=label, showarrow=False, xanchor="left", font={"size": 11, "color": FAINT}
    )
    fig.add_annotation(
        x=1,
        y=-1.1,
...
def verdict(text: str, high: bool) -> None:
    ...
    st.markdown(
        f'<p class="verdict"><span class="dot" style="background:{colour}"></span>{text}</p>',
        unsafe_allow_html=True,
    )


st.sidebar.markdown(
    '<p class="eyebrow" style="margin-bottom:.6rem">DengueShield</p>', unsafe_allow_html=True
)
screen = st.sidebar.radio(
    "Screen",
    ["Patient assessment", "Outbreak forecast", "About the models"],
...
if screen == "Patient assessment":
    header(
        "Patient assessment", "Individual risk", "Two independent estimates from routine bloodwork."
    )
    ...
    with tab1:
        ...
        if b is None:
            ...
            if mode == "Quick":
                ...
                st.markdown(
                    '<p class="note">Unentered values default to the study cohort median.</p>',
                    unsafe_allow_html=True,
                )
            else:
                ...
                vals["Age"] = c1.number_input("Age", 1, 100, 32)
                vals["Gender"] = 1 if c1.selectbox("Sex", ["Male", "Female"]) == "Male" else 0
                vals["Hemoglobin(g/dl)"] = c1.number_input(
                    "Haemoglobin (g/dl)", 5.0, 22.0, 14.2, 0.1
                )
            ...
            if dual is not None:
                district = st.selectbox(
                    "District (optional, adds local outbreak context)",
            ...
            if st.button("Assess", type="primary", key="screen_btn"):
                ...
                with a:
                    ...
                    verdict("Prioritise dengue testing" if flag else "Lower testing priority", flag)
                bcol.plotly_chart(
                    probability_bar(p, thr, "model-estimated probability"), width="stretch"
                )
                ...
                st.markdown(
                    f'<p class="note" style="margin-top:1rem">{DISCLAIMER}'
    ...
    with tab2:
        ...
        if b is None:
            ...
            st.markdown(
                '<p class="note">For a patient already known or suspected to have '
                "dengue. Estimated relative to the training population of 303 "
                "paediatric admissions.</p>",
                unsafe_allow_html=True,
            )
            c1, c2, c3 = st.columns(3)
            with c1:
            ...
            if st.button("Assess complication risk", type="primary", key="peds_btn"):
                row = {
                    ...
                    "LOWEST WBC": lo_wbc,
                    "HIGHEST WBC": hi_wbc,
                    "PLATELET COUNT": p_plt,
                    "AGE": p_age,
                    "GENDER": 1 if p_sex == "Male" else 0,
                    "HEADACHE": int(sx["headache"]),
                    "MYALGIA": int(sx["myalgia"]),
                    "ABDOMINAL PAIN": int(sx["abdominal pain"]),
                    "RASH": int(sx["rash"]),
                ...
                bcol.plotly_chart(probability_bar(p, thr, "estimated risk"), width="stretch")
                st.markdown(
                    ...
                    f"extra observation. {DISCLAIMER}</p>",
                    unsafe_allow_html=True,
                )
    ...
    if dual is None:
        ...
        for g in order:
            ...
            bar.add_trace(
                go.Bar(
                    ...
                    orientation="h",
                    name=f"{g} ({counts[g]})",
                    marker_color=TRIAGE[g][0],
                    hovertemplate=f"{g}: {counts[g]} districts<extra></extra>",
                    text=[str(counts[g])],
                    textposition="inside",
                    insidetextfont={
                        "color": readable_on(TRIAGE[g][0]),
        ...
        for col, g in zip(cols, order, strict=True):
            ...
            with col:
                ...
                if rows.empty:
                    st.markdown(
                        '<p class="note" style="font-size:.82rem"><em>No districts.</em></p>',
                        unsafe_allow_html=True,
                    )
                else:
                    listing = "".join(
                    ...
                    st.markdown(
                        '<p class="note" style="font-size:.74rem;'
        ...
        with right:
            ...
            st.dataframe(
                show.style.format(
                    {"Cases": "{:,.0f}", "Continuation": "{:.2f}", "Emergence": "{:.2f}"},
                    na_rep="—",
                ),
                hide_index=True,
                width="stretch",
                ...
                column_config={
                    ...
                    "Continuation": st.column_config.NumberColumn(
                        width="small", help="Probability an existing outbreak persists 14 days"
                    ),
                    "Emergence": st.column_config.NumberColumn(
                        width="small", help="Probability a new outbreak begins within 1-4 weeks"
                    ),
                },
            )
            st.markdown(
        ...
        if hist is not None:
            ...
            f.add_trace(
                go.Scatter(
                    x=h.week_start, y=h.casos, name="Cases", line={"width": 2, "color": ACCENT}
                )
            )
            f.update_layout(
                **PLOT_LAYOUT,
...
# ============================================================== 3. ABOUT
    ...
    with t1:
        ...
        st.code(
            """high risk flagged (14-day forecast)
        |
...
hospital preparedness raised""",
        ...
        st.markdown(
            '<p class="note">Decision support for prioritisation. A flag is an '
    ...
    with t2:
        ...
        if rb and "rolling_origin" in rb:
            ...
            f.update_layout(
                **PLOT_LAYOUT,
                height=300,
                title={
                    "text": "Retrained each year, tested on the next",
                    "y": 0.97,
                    "yanchor": "top",
                },
        ...
        finding(
            "Attribution is not incremental value",
    ...
    with t3:
        ...
        with st.expander("Error breakdown and transfer experiment"):
            ...
            if cal and "strata" in cal and "baseline_inc_q" in cal["strata"]:
                ...
                st.dataframe(
                    pd.DataFrame(cal["strata"]["baseline_inc_q"]).round(3),
                    hide_index=True,
                    width="stretch",
                )
                ...
                bars = pd.DataFrame(
                    {
                        ...
                        "pr_auc": [
                            0.910,
                            _v("2_brazil_zeroshot"),
                            _v("1_persistence"),
                            _v("3_srilanka_only"),
                            _v("4_brazil_finetuned"),
                        ],
                    }
                )
                f = px.bar(
    ...
    with t4:
        finding(
            "Three of four public dengue screening datasets are synthetic",
        ...
        st.markdown(
            '<p class="note">The rule now applied before modelling: any single '
        ...
        if au is not None:
            cols = [
                c
                for c in ["dataset", "n", "prevalence", "max_single_feature_auc", "verdict"]
                if c in au.columns
            ]
        ...
        st.dataframe(
            pd.DataFrame(
                [
                    ...
                    {
                        "Layer": "Outbreak, Brazil",
                        "Source": "Zenodo 22029053",
                        "Detail": "4.7M municipality-weeks, 2010–2025",
                    },
                    {
                        "Layer": "Outbreak, Sri Lanka",
                        "Source": "denguedatahub + Epidemiology Unit",
