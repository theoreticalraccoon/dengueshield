...
PARAMS = {"objective": "binary", "learning_rate": 0.03, "num_leaves": 127,
...
def run(rebuild):
    ...
    return {"rows": len(s), "n_test": len(te), "test_prevalence": float(te.y.mean()),
...
before = run(False)
print(f"  BEFORE (published positional lags): PR-AUC={before['pr_auc']:.4f} "
      f"acc={before['accuracy']:.4f} recall={before['recall']:.4f}", flush=True)
after = run(True)
print(f"  AFTER  (date-exact rebuilt lags)  : PR-AUC={after['pr_auc']:.4f} "
      f"acc={after['accuracy']:.4f} recall={after['recall']:.4f}", flush=True)
...
with open("reports/lag_fix_impact.json", "w") as _f:
    json.dump({"before_positional_lags": before, "after_date_exact_lags": after,
               "delta": delta}, _f, indent=2, default=float)
