...
from dengue.model2_outbreak import (load_panel, build_supervised, temporal_split,
                                    DYNAMIC, STATIC, HORIZON)
from dengue.lstm_outbreak import make_sequences, train_lstm, predict, SEQ_LEN

print("loading panel ...", flush=True)
sup = build_supervised(load_panel())
dyn = [c for c in DYNAMIC + ["week_sin","week_cos","casos_growth_1","casos_growth_4","inc_roll4"] if c in sup.columns]
...
sc_d.fit(imp_d.fit_transform(tr[dyn])); sc_s.fit(imp_s.fit_transform(tr[stat]))
...
print("building sequences on the FULL contiguous panel, then splitting by label year ...", flush=True)
...
Xtr,Str,ytr,_ = out["train"]; Xva,Sva,yva,_ = out["val"]; Xte,Ste,yte,ite = out["test"]
print("training LSTM ...", flush=True)
model, best_ap = train_lstm(Xtr,Str,ytr, Xva,Sva,yva, epochs=40, bs=512, patience=6)

pv = predict(model, Xva, Sva); pt = predict(model, Xte, Ste)
...
res = {"model":"PyTorch_LSTM","threshold":float(thr),
       "roc_auc":roc_auc_score(yte,pt),"pr_auc":average_precision_score(yte,pt),
       "accuracy":accuracy_score(yte,yh),"recall":recall_score(yte,yh,zero_division=0),
       "precision":precision_score(yte,yh,zero_division=0),"f1":f1_score(yte,yh,zero_division=0),
       "specificity":tn/max(tn+fp,1),"brier":brier_score_loss(yte,pt),
       "tp":int(tp),"fp":int(fp),"fn":int(fn),"tn":int(tn)}
print("\n=== LSTM TEST (2024-25) ===")
for k,v in res.items(): print(f"  {k:12} {v}")
torch.save({"state_dict":model.state_dict(),"dyn":dyn,"stat":stat,"seq_len":SEQ_LEN,
            "threshold":float(thr),"horizon":HORIZON}, "models/model2_lstm.pt")
joblib.dump({"imp_d":imp_d,"sc_d":sc_d,"imp_s":imp_s,"sc_s":sc_s}, "models/model2_lstm_scalers.joblib")
json.dump(res, open("reports/model2_lstm.json","w"), indent=2, default=float)
