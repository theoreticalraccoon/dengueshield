...
def load_cases() -> pd.DataFrame:
    ...
    return d.groupby(["district", "week_start"], as_index=False).agg(casos=("cases", "sum"))
...
def build_panel() -> pd.DataFrame:
    ...
    wk = (
        w.groupby(["district", "week_start"])
        .agg(
            precip_total_semana=("precipitation_sum", "sum"),
    ...
    meta = pd.DataFrame(
        [{"district": k, "population_total": v[2], "area_km2": v[3]} for k, v in DISTRICTS.items()]
    )
