...
def split(df, col="anio"):
    return (
        df[df[col] <= TRAIN_END],
        df[(df[col] > TRAIN_END) & (df[col] <= VAL_END)],
        df[df[col] > VAL_END],
    )
...
print(
    f"  panel: {sl_panel.shape}  districts={sl_panel.district.nunique()}  "
    f"{sl_panel.week_start.min().date()} -> {sl_panel.week_start.max().date()}",
    flush=True,
)

inc = sl_panel.p_inc100k
print(
    f"  weekly incidence per 100k: median={inc.median():.1f} p90={inc.quantile(0.9):.1f} "
    f"p99={inc.quantile(0.99):.1f} max={inc.max():.1f}"
)
...
if frac_over_br < 0.04:
    ...
    print(
        f"  -> Brazil threshold is near-degenerate here; adding a Sri Lanka-calibrated "
...
print(
    f"  common features used: {len(feats)}"
    + (f"  (missing in Brazil: {missing})" if missing else "")
)
...
for tname, thr_inc in sl_thresholds.items():
    ...
    print(
        f"  train={len(sl_tr)} val={len(sl_va)} test={len(sl_te)}  "
    ...
    p_te = sl_model.predict(sl_te[feats])
    ...
    ft = lgb.train(
        {**PARAMS, "scale_pos_weight": spw_sl, "learning_rate": 0.02},
        sl_ds,
        num_boost_round=300,
        init_model=br_model,
    )
    p_va = ft.predict(sl_va[feats])
    p_te = ft.predict(sl_te[feats])
    ...
    print(
        df[["condition", "roc_auc", "pr_auc", "accuracy", "recall", "precision", "f1"]]
        .round(4)
...
with open("reports/srilanka_transfer.json", "w") as _f:
    json.dump(all_rows, _f, indent=2, default=float)
