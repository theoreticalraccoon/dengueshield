...
def main() -> int:
    ...
    for rep in reports[:WEEKS]:
        ...
        if not df.empty:
            frames.append(df)
            print(
                f"  wk {rep['week']:>2} -> {df.week_start.iloc[0].date()}  "
                f"{len(df)} districts, {int(df.cases.sum()):,} cases",
                flush=True,
            )
    if not frames:
        ...
        json.dumps(
            {
                ...
                "latest_week_start": str(latest.date()),
                ...
                "self_check": check["verdict"],
                "national_total": int(cur.cases.sum()),
            },
            indent=2,
            default=str,
        )
    ...
    print(
        cur.nlargest(6, "cases")[["district", "cases", "prev_week", "change_pct"]]
        .round(1)
