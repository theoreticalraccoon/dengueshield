...
print(
    ...
    df.shape,
    "| municipios:",
    df.codigo_ibge.nunique(),
    "|",
    df.data_iniSE.min().date(),
    "->",
    df.data_iniSE.max().date(),
)
sup = build_supervised(df)
feats = feature_columns(sup)
print("supervised:", sup.shape, "| features:", len(feats), f"| outbreak rate: {sup.y.mean():.4f}")
...
rows.append(
    report(yte, clim, best_threshold(yva, seasonal_climatology(tr, va)), "BASELINE_climatology")
)
...
with open("reports/model2_summary.json", "w") as _f:
    json.dump(res.to_dict("records"), _f, indent=2, default=float)
