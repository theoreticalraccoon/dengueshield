...
def test_an_unreadable_bundle_loads_as_none_instead_of_raising(tmp_path, monkeypatch):
    """A pickle the environment cannot read must not take down the whole app.

    joblib bundles are pickles, so they fail when the reading stack is not
    compatible with the writing one - a numpy floor loose enough to resolve 1.26
    against models written under numpy 2 was enough to crash every screen on
    startup, including the About page that would have explained it.
    """
    monkeypatch.setattr(P, "MODELS", tmp_path)
    corrupt = tmp_path / P.FILENAMES[P.SL_EMERGENCE]
    corrupt.write_bytes(b"\x80\x04 not a pickle at all")

    assert P.load(P.SL_EMERGENCE) is None


def test_an_unreadable_bundle_records_why(tmp_path, monkeypatch):
    """None alone cannot be distinguished from "never trained", and the fix differs."""
    monkeypatch.setattr(P, "MODELS", tmp_path)
    corrupt = tmp_path / P.FILENAMES[P.SL_EMERGENCE]
    corrupt.write_bytes(b"\x80\x04 not a pickle at all")

    P.load(P.SL_EMERGENCE)
    assert P.SL_EMERGENCE in P.LOAD_ERRORS
    assert P.LOAD_ERRORS[P.SL_EMERGENCE]


def test_a_missing_bundle_is_not_recorded_as_an_error(tmp_path, monkeypatch):
    """Absent is normal on a clean checkout; it must not raise a scary banner."""
    monkeypatch.setattr(P, "MODELS", tmp_path)
    assert P.load(P.SL_EMERGENCE) is None
    assert P.SL_EMERGENCE not in P.LOAD_ERRORS


def test_a_stale_error_clears_once_the_bundle_loads(tmp_path, monkeypatch):
    """Otherwise the banner outlives the problem it describes."""
    monkeypatch.setattr(P, "MODELS", tmp_path)
    corrupt = tmp_path / P.FILENAMES[P.SL_EMERGENCE]
    corrupt.write_bytes(b"\x80\x04 not a pickle at all")
    P.load(P.SL_EMERGENCE)
...
def test_one_broken_bundle_does_not_stop_the_others_loading(tmp_path, monkeypatch):
    """load_all iterates; the first failure used to abort the rest."""
    monkeypatch.setattr(P, "MODELS", tmp_path)
    (tmp_path / P.FILENAMES[P.SCREENING]).write_bytes(b"\x80\x04 broken")

    loaded = P.load_all()
    assert set(loaded) == set(P.FILENAMES)
    assert loaded[P.SCREENING] is None
