...
from dengue.model2_outbreak import load_panel, feature_columns

HERE = Path(__file__).parent
SEED = 42
PARAMS = {"objective": "binary", "learning_rate": 0.03, "num_leaves": 63, "min_child_samples": 60, "subsample": 0.85, "subsample_freq": 1, "colsample_bytree": 0.8, "reg_lambda": 10.0, "verbose": -1, "seed": SEED}
...
def build_emergence(df, horizon=4, outbreak_inc=100.0, quiet_weeks=2):
    ...
    df = df.drop_duplicates(["codigo_ibge", "data_iniSE"]).sort_values(
        ["codigo_ibge", "data_iniSE"]).copy()
...
def fit_eval(tr, va, te, feats, name):
    ...
    m = lgb.train({**PARAMS, "scale_pos_weight": spw},
                  lgb.Dataset(tr[feats], label=tr.y.values), num_boost_round=600)
    pv, pt = m.predict(va[feats]), m.predict(te[feats])
    return m, score(te.y.values, pt, tune_thr(va.y.values, pv), name,
...
for HORIZON, INC in [(4, 100.0), (2, 100.0), (4, 50.0)]:
    ...
    rows = []
    # ---- trivial baselines ----
    for bname, col in [("baseline_persistence", "base_persistence"),
                       ("baseline_growth_rate", "base_growth"),
                       ("baseline_moving_average", "base_movavg")]:
        pv, pt = va[col].values, te[col].values
        rows.append(score(te.y.values, pt, tune_thr(va.y.values, pv), bname,
                          {"n_features": 1}))
        print(f"  {bname:26} PR-AUC={rows[-1]['pr_auc']:.4f}  "
              f"recall={rows[-1]['recall']:.4f}  lift={rows[-1]['lift_over_prevalence']:.2f}x",
              flush=True)

    # ---- feature-set ablation ----
    for mname, cols in [("model_incidence_only", HIST),
                        ("model_environment_only", ENV),
                        ("model_combined", feats)]:
        _, r = fit_eval(tr, va, te, cols, mname)
        rows.append(r)
        print(f"  {mname:26} k={len(cols):>2}  PR-AUC={r['pr_auc']:.4f}  "
              f"recall={r['recall']:.4f}  lift={r['lift_over_prevalence']:.2f}x", flush=True)
    ...
    results[tag + "_summary"] = {
        "incidence_only": float(inc_o), "environment_only": float(env_o),
        "combined": float(comb), "best_baseline": float(best_base),
        "env_gain_over_incidence": float(comb - inc_o),
        "model_gain_over_baseline": float(comb - best_base),
        "test_prevalence": float(te.y.mean()), "n_positive": int(te.y.sum()),
    }

HERE.mkdir(parents=True, exist_ok=True)
with open(HERE / "emergence_results.json", "w") as _f:
    json.dump(results, _f, indent=2, default=float)
flat = pd.concat([pd.DataFrame(v).assign(config=k)
                  for k, v in results.items() if isinstance(v, list)], ignore_index=True)
