...
from dengue.ablation import HEMA_GROUPS, PEDS_GROUPS, load_peds, run_ablation
...
Xp, yp, mp = load_peds()
print(
    f"n={len(yp)}  complicated={yp.sum()} ({yp.mean():.3f})  trivial_acc={max(yp.mean(), 1 - yp.mean()):.4f}"
)
rep = audit("peds", Xp, yp)
print(f"audit: {rep['verdict']}  max_single_feature_auc={rep['max_single_feature_auc']:.4f}")
pn = permutation_null(
    LGBMClassifier(n_estimators=200, num_leaves=7, random_state=42, n_jobs=4, verbose=-1),
    Xp,
    yp,
    n=8,
    folds=5,
...
print(
    f"permutation control: real={pn['real_auc']:.4f} null={pn['null_mean']:.4f}+/-{pn['null_sd']:.4f} z={pn['z']:.1f}"
)
print("\nLightGBM:")
...
out["peds"] = {
    "audit": {k: v for k, v in rep.items() if k != "single_feature_auc"},
    "permutation": pn,
    "ablation": abl_p.to_dict("records"),
}
...
print(
    f"n={len(yh)}  positive={yh.sum()} ({yh.mean():.3f})  trivial_acc={max(yh.mean(), 1 - yh.mean()):.4f}"
)
...
with open("reports/ablation_summary.json", "w") as _f:
    json.dump(out, _f, indent=2, default=float)
