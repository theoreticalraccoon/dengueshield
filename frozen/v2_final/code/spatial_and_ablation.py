...
# ---- information sources -------------------------------------------------------
HISTORICAL = ["casos", "p_inc100k", "Rt", "casos_lag_1", "casos_lag_2", "casos_lag_4",
              "casos_lag_8", "Rt_lag_1", "Rt_lag_2", "Rt_lag_4", "casos_roll4_mean",
              "casos_roll8_mean", "casos_growth_1", "casos_growth_4", "inc_roll4"]


def fit_predict(tr, va, te, feats):
    spw = (tr.y.values == 0).sum() / max((tr.y.values == 1).sum(), 1)
    m = lgb.train({**PARAMS, "scale_pos_weight": spw},
...
def score(y, p, thr, name, extra=None):
    ...
    d = {"condition": name,
         ...
         "n_test": int(len(y)), "test_prevalence": float(y.mean()),
...
rng.shuffle(munis)
n_hold = int(round(0.20 * len(munis)))
...
tr = seen[seen.anio <= 2021]; va = seen[(seen.anio > 2021) & (seen.anio <= 2023)]
...
print(f"  A temporal, SEEN munis      PR-AUC={rows[-1]['pr_auc']:.4f} acc={rows[-1]['accuracy']:.4f}")
...
print(f"  B spatio-temporal, UNSEEN   PR-AUC={rows[-1]['pr_auc']:.4f} acc={rows[-1]['accuracy']:.4f}")
...
print(f"  C spatial only, UNSEEN      PR-AUC={rows[-1]['pr_auc']:.4f} acc={rows[-1]['accuracy']:.4f}")

# (d) persistence on the unseen spatiotemporal rows
rows.append(score(te_unseen.y.values, te_unseen.baseline_persistence.values, 0.5,
                  "D_persistence_unseen_municipalities"))
print(f"  D persistence on UNSEEN     PR-AUC={rows[-1]['pr_auc']:.4f} acc={rows[-1]['accuracy']:.4f}")
...
for i, hs in enumerate(folds):
    ...
    r = score(s_te.y.values, spt, tune_thr(s_va.y.values, spv),
              f"state_fold_{i}", {"held_states": sorted(hs)})
    state_rows.append(r)
    print(f"    fold {i}: {len(hs)} states, n={r['n_test']:>6}  PR-AUC={r['pr_auc']:.4f} "
          f"acc={r['accuracy']:.4f} recall={r['recall']:.4f}", flush=True)
if state_rows:
    sdf = pd.DataFrame(state_rows)
    print(f"    MEAN over state folds: PR-AUC={sdf.pr_auc.mean():.4f} acc={sdf.accuracy.mean():.4f}")
...
tr = sup[sup.anio <= 2021]; va = sup[(sup.anio > 2021) & (sup.anio <= 2023)]; te = sup[sup.anio > 2023]
abl = []
for name, cols in [("A_historical_only", HIST), ("B_environmental_only", ENV),
    ...
    abl.append(r)
    print(f"  {name:32} k={len(cols):>2}  PR-AUC={r['pr_auc']:.4f}  acc={r['accuracy']:.4f}  "
          f"recall={r['recall']:.4f}", flush=True)
abl.append(score(te.y.values, te.baseline_persistence.values, 0.5,
                 "D_persistence_baseline", {"n_features": 1}))
print(f"  {'D_persistence_baseline':32} k= 1  PR-AUC={abl[-1]['pr_auc']:.4f}  "
...
print(f"  history adds {results['ablation_summary']['historical_gain_over_persistence']:+.4f} "
      f"PR-AUC beyond persistence")
print(f"  environment ALONE vs persistence: "
      f"{results['ablation_summary']['environment_alone_vs_persistence']:+.4f}")

json.dump(results, open("reports/model2_spatial_ablation.json", "w"), indent=2, default=float)
