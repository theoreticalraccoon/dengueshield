...
PEDS_GROUPS = {
    ...
    "C_+symptoms":      ["HEADACHE", "MYALGIA", "ABDOMINAL PAIN", "RASH", "VOMITING",
                         "BREATHLESSNESS", "BLEEDING", "ORGANOMEGALY", "BP AT ADMISSION"],
...
def load_peds():
    ...
    return X, y, {"name": "peds_complications", "source": "Zenodo 6476112",
                  "task": "complicated dengue (needs closer monitoring)"}


# ------------------------------------------------------------- screening cohort
HEMA_GROUPS = {
    "A_CBC": ["Hemoglobin(g/dl)", "Neutrophils(%)", "Lymphocytes(%)", "Monocytes(%)",
              "Eosinophils(%)", "RBC", "HCT(%)", "MCV(fl)", "MCH(pg)", "MCHC(g/dl)",
...
LGBM_GRID = {"clf__n_estimators": [100, 200, 400], "clf__learning_rate": [0.01, 0.03, 0.05],
             "clf__num_leaves": [3, 7, 15], "clf__min_child_samples": [5, 10, 20, 30],
             "clf__subsample": [0.7, 0.85, 1.0], "clf__subsample_freq": [1],
             "clf__colsample_bytree": [0.6, 0.8, 1.0], "clf__reg_lambda": [1, 5, 20]}


def nested_oof(X: pd.DataFrame, y: np.ndarray, kind: str = "lgbm",
               n_splits: int = 5, n_repeats: int = 3, calibrate: bool = True) -> np.ndarray:
    ...
    for r in range(n_repeats):
        ...
        for tr, te in outer.split(X, y):
            base = _model(kind)
            if kind == "lgbm":
                est = RandomizedSearchCV(base, LGBM_GRID, n_iter=25, scoring="roc_auc",
                                         cv=StratifiedKFold(4, shuffle=True, random_state=SEED),
                                         random_state=SEED, n_jobs=4, refit=True)
                est.fit(X.iloc[tr], y[tr]); est = est.best_estimator_
            else:
...
def full_metrics(y, p, thr) -> dict:
    ...
    return {"roc_auc": roc_auc_score(y, p), "pr_auc": average_precision_score(y, p),
            "brier": brier_score_loss(y, p), "threshold": float(thr),
            "accuracy": accuracy_score(y, yh), "sensitivity": sens, "specificity": spec,
            "ppv": ppv, "npv": npv, "balanced_accuracy": (sens + spec) / 2,
            "tp": int(tp), "fp": int(fp), "fn": int(fn), "tn": int(tn)}


def run_ablation(X: pd.DataFrame, y: np.ndarray, groups: dict[str, list[str]],
                 kind: str = "lgbm", sens_target: float = 0.90) -> pd.DataFrame:
    ...
    for label, add in groups.items():
        ...
        rows.append(m)
        print(f"  {label:20} k={len(cols):>2}  AUC={m['roc_auc']:.4f}  PR-AUC={m['pr_auc']:.4f}  "
