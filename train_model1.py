...
from dengue.model1_screening import SEED, build_models, metrics_at, threshold_for_sensitivity
...
for name, model in build_models().items():
    ...
    agg = {
        k: (np.mean([d[k] for d in per]), np.std([d[k] for d in per]))
        for k in [
            "roc_auc",
            "pr_auc",
            "accuracy",
    ...
    rows.append(
        {
            ...
            **{k + "_sd": agg[k][1] for k in agg},
            "acc@sens90": m90["accuracy"],
            "spec@sens90": m90["specificity"],
            "thr@sens90": thr,
        }
    )
    print(
        f"{name:20} AUC={agg['roc_auc'][0]:.4f}+/-{agg['roc_auc'][1]:.4f}  "
