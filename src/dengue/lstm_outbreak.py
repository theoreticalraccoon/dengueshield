...
def make_sequences(df: pd.DataFrame, dyn: list[str], stat: list[str], seq_len: int = SEQ_LEN):
    ...
    if not Xs:
        return (
            np.empty((0, seq_len, len(dyn)), np.float32),
            np.empty((0, len(stat)), np.float32),
...
class OutbreakLSTM(nn.Module):
    def __init__(
        self, n_dyn: int, n_stat: int, hidden: int = 128, layers: int = 2, p_drop: float = 0.3
    ):
        super().__init__()
        self.lstm = nn.LSTM(
            n_dyn,
        ...
        self.head = nn.Sequential(
            nn.Linear(hidden + 64, 128), nn.ReLU(), nn.Dropout(p_drop), nn.Linear(128, 1)
        )
...
def train_lstm(
    Xtr, Str, ytr, Xva, Sva, yva, epochs=40, bs=512, lr=1e-3, patience=6, device=None, seed=42
):
    ...
    dl = DataLoader(
        TensorDataset(torch.tensor(Xtr), torch.tensor(Str), torch.tensor(ytr)),
        batch_size=bs,
        shuffle=True,
        drop_last=True,
    )
...
def predict(model, X, S, device=None, bs=4096):
    ...
    for i in range(0, len(X), bs):
        ...
        sb = torch.tensor(S[i : i + bs], device=device)
