...
thr = float(
    max((f1_score(va.y.values, (p_va >= t).astype(int), zero_division=0), t) for t in qs)[1]
)
p_te = m_dev.predict(te[feats])
print(
    f"held-out test: PR-AUC={average_precision_score(te.y.values, p_te):.4f} "
...
].to_parquet("data/processed/srilanka_history.parquet", index=False)
