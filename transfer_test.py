...
from dengue.datasets import (
    load_bd_comprehensive,
    load_bd_structured,
    load_hematology_1523,
    load_vitals_1003,
)
...
def harmonise(X, y, m):
    ren = {
        "hematology_1523": {
            "Age": "age",
            "Gender": "sex",
            "Total Platelet Count(/cumm)": "platelet",
...
for name, (X, y) in sets.items():
    p = cross_val_predict(
        mk(), X, y, cv=StratifiedKFold(5, shuffle=True, random_state=0), method="predict_proba"
    )[:, 1]
...
for src in ["bd_structured", "bd_comprehensive", "vitals_1003"]:
    ...
    yh = (p > 0.5).astype(int)
    print(
        f"  trained on {src:18} -> real data: AUC={roc_auc_score(yr, p):.4f} "
