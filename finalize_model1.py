...
from dengue.model1_screening import SEED, metrics_at, threshold_for_sensitivity
...
for a, b in zip(pp, pt, strict=True):
    print(f"    {a:.3f} -> {b:.3f}")
...
joblib.dump(
    {
        ...
        "threshold_sens90": float(thr90),
        "metrics_nested_cv": {
            "roc_auc": m05["roc_auc"],
            "pr_auc": m05["pr_auc"],
            "brier": m05["brier"],
            "accuracy@0.5": m05["accuracy"],
            "accuracy@sens90": m90["accuracy"],
            "specificity@sens90": m90["specificity"],
        },
    },
    "models/model1_screening.joblib",
)
with open("reports/model1_final.json", "w") as _f:
    json.dump(
        {
