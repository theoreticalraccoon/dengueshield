...
def integrity_of(df, label):
    ...
    d = {"label": label, "rows": int(len(df)),
         "clean_7day_steps": int((gap == 7).sum()),
         "duplicate_municipality_week_rows":
             int(df.duplicated(["codigo_ibge", "data_iniSE"]).sum()),
...
print(f"  FEATURE CUTOFF      : {(wk + pd.Timedelta(days=6)).date()}  "
      f"(all inputs observed on or before this date)")
print(f"  forecast issued     : {(wk + pd.Timedelta(days=7)).date()} "
      f"(once week t is closed)")
print(f"  TARGET WINDOW       : week t+{HORIZON} = "
...
print(f"  label               : incidence in target week >= {OUTBREAK_INC}/100k "
      f"-> y={int(ex.y)}")
out["worked_example"] = {
    "municipality": str(ex.municipio), "state": str(ex.estado),
    ...
    "lead_time_days": HORIZON * 7, "label": int(ex.y),
}
...
print("=" * 78)
s = panel[panel.codigo_ibge == panel.codigo_ibge.iloc[0]].drop_duplicates(
    ["codigo_ibge", "data_iniSE"]).sort_values("data_iniSE").reset_index(drop=True)
checks = {}
for L in (1, 2, 4, 8):
    col = f"casos_lag_{L}"
...
for L in (1, 2, 4, 8):
    ...
    fut += int((s.loc[m, f"casos_lag_{L}"].values == lead[m].values).sum()
...
def evaluate(delay_weeks):
    ...
    m = lgb.train({**PARAMS, "scale_pos_weight": spw},
                  lgb.Dataset(tr[f], label=tr.y.values), num_boost_round=700)
    ...
    return {"delay_weeks": delay_weeks, "n_test": int(len(te)),
...
for k in (0, 1, 2):
    r = evaluate(k)
    delays.append(r)
    print(f"  delay {k} weeks (effective lead {r['effective_lead_days']:>2}d): "
