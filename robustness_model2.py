...
for test_year in range(2019, 2026):
    tr = sup[sup.anio <= test_year - 2]
    va = sup[sup.anio == test_year - 1]
    te = sup[sup.anio == test_year]
    if len(te) == 0 or te.y.sum() < 10 or va.y.sum() < 5:
        continue
    r = evaluate(tr.y.values, tr[feats], va.y.values, va[feats], te.y.values, te[feats])
    r["test_year"] = test_year
    rows.append(r)
    print(
        f"  {test_year}: PR-AUC={r['pr_auc']:.4f} ROC={r['roc_auc']:.4f} acc={r['accuracy']:.4f} "
        f"(trivial {r['trivial_acc']:.4f}) recall={r['recall']:.4f} prec={r['precision']:.4f}",
        flush=True,
    )
bt = pd.DataFrame(rows)
results["rolling_origin"] = rows
print(
    f"  MEAN over years: PR-AUC={bt.pr_auc.mean():.4f}  acc={bt.accuracy.mean():.4f}  recall={bt.recall.mean():.4f}"
)
...
for h in [2, 4, 8]:
    ...
    rows.append(r)
    print(
        f"  h={h:>2}w: PR-AUC={r['pr_auc']:.4f} acc={r['accuracy']:.4f} (trivial {r['trivial_acc']:.4f}) "
...
for inc in [50.0, 100.0, 300.0]:
    ...
    rows.append(r)
    print(
        f"  >={inc:>5.0f}/100k: PR-AUC={r['pr_auc']:.4f} acc={r['accuracy']:.4f} "
        f"(trivial {r['trivial_acc']:.4f}) recall={r['recall']:.4f} prev={r['test_prevalence']:.4f}",
        flush=True,
    )
...
print(
    f"  shuffled-train: PR-AUC={r['pr_auc']:.4f} ROC={r['roc_auc']:.4f} "
    f"(chance PR-AUC ~= {te.y.mean():.4f})"
)
results["shuffled_control"] = r

with open("reports/model2_robustness.json", "w") as _f:
